    ...
    if (activeSession) {
      ...
      if (session?.profile_id) {
        fetch(`/api/chat/customer?profileId=${session.profile_id}`)
          ...
          .then((data) => {
            if (data.customer) {
              // Crew only sees non-confidential info
              setCustomerDetail({
                id: data.customer.id,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Customer chat messages</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
                ...
                {sessions.map((s) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between">
                        <p className="truncate text-sm font-medium text-gray-900">{s.customer_name}</p>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      {activeSessionData?.message_count || 0} messages
                    </p>
                  </div>
                </div>
                  ...
                  {messages.map((msg) => (
                    ...
                    >
                      <div className="max-w-[75%]">
                        {msg.sender === 'admin' && (
                          <p className="mb-0.5 text-right text-xs font-medium text-green-600">Yumi</p>
                        )}
                        <div
                          className={cn(
              ...
              <div className="border-t px-4 py-3">
                ...
                >
                  <input
                    ...
                    className="flex-1 rounded-full border border-gray-300 px-4 py-2.5 text-sm outline-none placeholder:text-gray-400 focus:border-green-400 focus:ring-1 focus:ring-green-400"
                    disabled={sending}
                  />
                  <button
                    type="submit"
                    disabled={!reply.trim() || sending}
                    className="flex h-10 w-10 items-center justify-center rounded-full bg-green-600 text-white transition-colors hover:bg-green-700 disabled:bg-gray-300"
                  >
        ...
        {/* Customer Detail Panel — crew version (no financial info) */}
                  ...
                  <div className="mb-3 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {customerDetail.phone && (
                        <p className="text-xs text-gray-500">{customerDetail.phone}</p>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="space-y-2">
                      {customerDetail.bookings.map((b) => (
                        <div key={b.id} className="rounded-lg border border-gray-200 p-2.5">
                          ...
                          {b.service_notes && (
                            <p className="mt-1 text-xs italic text-gray-400">{b.service_notes}</p>
                          )}
                        </div>
                      ))}
                    </div>
